...
public class TestModifier : MonoBehaviour, IDanmakuModifier
{
    ...
    public JobHandle UpdateDannmaku(DanmakuPool pool, JobHandle dependency = default(JobHandle))
    {
        ...
        return new ApplyWave(){NewSpeed = BaseSpeed+Mathf.Sin(Time.time*1f/WaveLength)*WaveSpeed, Speeds = pool.Speeds}.Schedule(pool.ActiveCount, DanmakuPool.kBatchSize, dependency);
